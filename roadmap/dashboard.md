...
| Go CLI (`ai-session`) — CLI subcommands | ✅ Done | 16 subcommands: `create-feature`, `resolve-feature-dir`, `append-log`, `update-task`, `update-slice`, `plan-list`, `plan-get`, `plan-write`, `plan-enrich-task`, `plan-split-task`, `load-context`, `implement`, `review`, `review-write`, `review-update`, `address-feedback`. Cobra CLI, yaml.Node API, testify. Makefile with build/test/lint. `plan-write` validates schema + writes atomically; `plan-enrich-task` updates single task field with injection guard and status lock; `plan-split-task` replaces one todo task with N atomic subtasks; `load-context` outputs feature dir files as XML blocks; `implement` is the Go orchestrator for the implementation phase (see `/session:implement` row). `review` fetches a diff and pipes it to a headless LLM prompt — supports `--strategy=branch` (full branch diff vs origin, default) or `--strategy=last-commit` (uncommitted changes vs HEAD); both strategies include untracked files. `review-write` validates and atomically writes review findings from stdin. `review-update` atomically updates the status and optional notes of multiple findings in a single write operation. `ai-session address-feedback sc-1234 [--regular] [--docs] [--devops] [--remote]`
