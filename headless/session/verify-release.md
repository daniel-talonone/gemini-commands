...
The user has provided the following arguments for the verification: `{{args}}`

**Steps:**

1.  **Execute Verification Script:**
    *   Use the `run_shell_command` tool to execute the `verify-release.sh` script, passing the user's arguments to it.
    *   Example: `$AI_SESSION_HOME/scripts/verify-release.sh {{args}}`
