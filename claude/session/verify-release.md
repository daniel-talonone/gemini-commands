---
description: Verifies a cherry-picked release on the current branch, providing an AI-powered analysis of any changes found.
---
...
The user has provided the following arguments for the verification: `$ARGUMENTS`

**Steps:**

1.  **Execute Verification Script:**
    *   Use the Bash tool to execute the `verify-release.sh` script, passing the user's arguments to it.
    *   Example: `$AI_SESSION_HOME/scripts/verify-release.sh $ARGUMENTS`
...
2.  **Analyze the JSON Output:**
    ...
    *   **If the status is "VERIFICATION_SUCCESSFUL"**: Report success to the user and stop.
    *   **If the status is "VERIFICATION_FAILED"**: Parse the other fields and present a clear, analyzed report covering:
...
3.  **Present Final Report:**
    *   Combine all findings into a single, well-structured Markdown report.
    *   Start with "VERIFICATION FAILED".
