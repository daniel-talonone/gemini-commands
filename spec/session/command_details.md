...
-   **Dependencies:**
    -   **Skills:** None
    -   **Scripts:** `ai-session append-log`, `ai-session update-task`
...
-   **Dependencies:**
    -   **Skills:** None
    -   **Scripts:** `ai-session create-feature`, `ai-session resolve-feature-dir`
...
## `/session:implement {{args}}`

-   **Description:** Executes all tasks in `plan.yml` slice by slice, autonomously and
    without user interaction. Runs an initial verification gate before touching any code,
    then verifies after each task. On verification failure, attempts to fix the issue up
    to 5 times before stopping. Logs all exit conditions to `log.md`.
-   **Orchestration Pattern:** Headless only (hand-written — not generated by gen_headless.sh)
-   **Dependencies:**
    -   **Scripts:** `ai-session resolve-feature-dir`, `ai-session append-log`, `ai-session update-task`
    -   **Tools:** `run_shell_command`, `glob`, `grep_search`
    -   **External Services:** None
-   **Inputs:**
    -   `{{args}}` — feature ID or path, passed to `ai-session resolve-feature-dir`
    -   `$FEATURE_DIR/plan.yml` — the implementation plan to execute
    -   `AGENTS.md` in the project CWD — must contain a `## Verification` section with a `Run:` line
-   **Outputs:**
    -   Modifies project source files to implement each task.
    -   Updates task and slice `status` fields in `$FEATURE_DIR/plan.yml` via `yq`.
    -   Appends structured exit entries to `$FEATURE_DIR/log.md`.
-   **Exit Conditions:**
    -   `IMPLEMENT COMPLETE` — all slices done, ready for `/session:review`
    -   `IMPLEMENT STOPPED: Initial verification failed` — codebase was broken before changes
    -   `IMPLEMENT STOPPED: Task failed after 5 attempts` — task marked `in-progress`, human needed
    -   `IMPLEMENT STOPPED: Ambiguity` — unresolvable task reference, human needed
    -   `IMPLEMENT STOPPED: Dependency not done` — depends_on not satisfied
