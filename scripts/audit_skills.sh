#!/usr/bin/env bash
# audit_skills.sh — Audit claude/session/*.md for stale ai-session CLI references and gap comments.
#
# Checks:
#   1. Every `ai-session <subcommand>` invocation — verifies the subcommand still exists in the CLI.
#   2. Gap comments ("# No CLI command yet") — surfaces them so stale ones can be upgraded.
#
# By default, the raw report is piped through `claude -p` for interpretation and fix suggestions.
# Use --raw to skip the LLM step and print the report only.
#
# Usage:
...
for md_file in "$CLAUDE_DIR"/*.md; do
    ...
    while IFS= read -r match; do
        ...
        if ! subcommand_exists "$subcommand"; then
            report+="[STALE] $name.md:$lineno — unknown subcommand '$subcommand'\n"
            report+="        $(echo "$line" | sed 's/^[[:space:]]*//')\n"
            stale_count=$((stale_count + 1))
        elif [ "$VERBOSE" -eq 1 ]; then
            report+="[OK]    $name.md:$lineno — ai-session $subcommand\n"
        fi
...
for md_file in "$CLAUDE_DIR"/*.md; do
    ...
    while IFS= read -r match; do
        lineno="${match%%:*}"
        line="${match#*:}"
        report+="[DIRECT-READ] $name.md:$lineno\n"
        report+="              $(echo "$line" | sed 's/^[[:space:]]*//')\n"
...
3. For each [DIRECT-READ] entry: judge whether the reference is an actual direct file read that bypasses the CLI, or benign (e.g. a comment, a CLI argument placeholder like \`{{feature-dir}}/review.yml\`). If it's a real bypass, flag it and suggest the correct CLI replacement (\`ai-session load-context <feature-id> | grep -A N 'filename'\`).
4. Output a prioritized fix list grouped as:
...
5. For each fix, show the exact change: old line → new line.
